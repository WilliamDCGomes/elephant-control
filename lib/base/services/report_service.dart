...
class ReportService extends BaseService {
  Future<ReportViewController?> getDefaultReport(DateTime beginDate, DateTime endDate, List<String> machineId) async {
    try {
      ...
      final response = await get(url, query: {"BeginDate": beginDate.toString(), "EndDate": endDate.toString(), "MachineId": machineId.toList()}, headers: {'Authorization': 'Bearer ${token}'});
  ...
  Future<ReportViewController?> getClosingReport(DateTime closingReportDateFilter, List<String> machineId) async {
    try {
      ...
      final response = await get(url, query: {"ClosingReportDateFilter": closingReportDateFilter.toString(), "MachineId": machineId.toList()}, headers: {'Authorization': 'Bearer ${token}'});
      if (hasErrorResponse(response)) throw Exception();
      return ReportViewController.fromJson(response.body);
    } catch (_) {
      return null;
